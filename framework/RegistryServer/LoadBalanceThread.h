 ...
 *  处理异步操作的线程
 */
class LoadBalanceThread : public TC_Thread,
                          public TC_Singleton<LoadBalanceThread> {
 ...
 public:
  ...
  virtual ~LoadBalanceThread() { terminate(); }
  ...
  // 获取节点权重
  void getDynamicWeight(const string &servant,
                        std::vector<tars::EndpointF> &vtEndpoints);

 private:
  // 更新动态负载权重
  void updateDynamicWeight();
  // 从DB加载负载数据
  void getMonitorDataFromDB(const vector<string> &vtServer,
                            LoadCache &loadCache);
  // 单节点获取权重
  void getDynamicWeight(const vector<LoadBalanceItem> &vtLoadBalanceInfo,
