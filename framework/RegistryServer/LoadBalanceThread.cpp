...
void LoadBalanceThread::init() {
  ...
  _loadInterval = TC_Common::strto<int>(
      g_pconf->get("/tars/loadbalance<loadinterval>", "120"));
  map<string, string> mapLoadProportion;
  if (g_pconf->getDomainMap("/tars/loadbalance/loadproportion",
                            mapLoadProportion)) {
    ostringstream log;
  ...
  _dynamicSwitch =
      TC_Common::strto<int>(g_pconf->get("/tars/loadbalance<loadswitch>", "0"));

  TLOGDEBUG("init _dynamicSwitch: " << _dynamicSwitch << "|"
                                    << "_loadInterval: " << _loadInterval
                                    << endl);
}

void LoadBalanceThread::terminate() { _terminate = true; }
...
void LoadBalanceThread::getMonitorDataFromDB(const vector<string> &vtServer,
                                             LoadCache &loadCache) {
  ...
  if (LOAD_BALANCE_DB_SUCCESS == _db.loadStatData(vtServer, statData)) {
    ...
    for (decltype(SIZE) i = 0; i < SIZE; ++i) {
      ...
      info.timeoutCount =
          TC_Common::strto<int64_t>(statData[i]["timeout_count"]);
...
void LoadBalanceThread::getDynamicWeight(
    const string &servant, std::vector<tars::EndpointF> &vtEndpoints) {
  if (servant.empty()) {
    return;
  }
  ...
  if (loadCache.end() != iter)  // 动态负载的key是servant或者服务名
  {
    ...
    for (size_t i = 0; i < SIZE; ++i) {
      ...
      log << endpoint.host << ":" << endpoint.port << ", "
          << endpoint.weightType << ", " << endpoint.weight << ", "
...
void LoadBalanceThread::getDynamicWeight(
    const vector<LoadBalanceItem> &vtLoadBalanceInfo,
    tars::EndpointF &endpointF) {
  for (const auto &info : vtLoadBalanceInfo) {
    if (info.slaveIp == endpointF.host) {
      endpointF.weightType =
          LOAD_BALANCE_STATIC_WEIGHT;  // 这里赋值为静态权重类型是因为直接复用EndpointManager里面的静态权重算法
  ...
  for (auto &loadPair : loadCache) {
    ...
    for (auto &loadInfo : loadPair.second.vtBalanceItem) {
      ...
      if (mapIp2LoadInfo.end() == iter) {
        ...
        log << loadInfo.slaveIp << ", " << loadInfo.timeoutCount << ", "
            << loadInfo.succCount << ", " << loadInfo.aveTime << "; ";
      }
...
void LoadBalanceThread::calculateWeight(LoadCache &loadCache) {
  for (auto &loadPair : loadCache) {
    ...
    const auto ITEM_SIZE(
        static_cast<int>(loadPair.second.vtBalanceItem.size()));
    int aveTime(loadPair.second.aveTimeSum / ITEM_SIZE);
    log << "aveTime: " << aveTime << "|"
        << "vtBalanceItem size: " << ITEM_SIZE << "|";
    for (auto &loadInfo : loadPair.second.vtBalanceItem) {
      // 按每台机器在总耗时的占比反比例分配权重：权重 = 初始权重 *（耗时总和 -
      // 单台机器平均耗时）/ 耗时总和
      TLOGDEBUG("loadPair.second.aveTimeSum: " << loadPair.second.aveTimeSum
                                               << endl);
      int aveTimeWeight(loadPair.second.aveTimeSum
                            ? (DEFAULT_WEIGHT * ITEM_SIZE *
                               (loadPair.second.aveTimeSum - loadInfo.aveTime) /
                               loadPair.second.aveTimeSum)
                            : 0);
      aveTimeWeight = aveTimeWeight <= 0 ? MIN_WEIGHT : aveTimeWeight;
      // 超时率权重：超时率权重 = 初始权重 - 超时率 * 初始权重 *
      // 90%，折算90%是因为100%超时时也可能是因为流量过大导致的，保留小流量试探请求
      int timeoutRateWeight(
          loadInfo.succCount
              ? (DEFAULT_WEIGHT -
                 static_cast<int>(loadInfo.timeoutCount *
                                  TIMEOUT_WEIGHT_FACTOR /
                                  (loadInfo.succCount + loadInfo.timeoutCount)))
              : (loadInfo.timeoutCount ? MIN_WEIGHT : DEFAULT_WEIGHT));
      // 各类权重乘对应比例后相加求和
      loadInfo.weight =
          aveTimeWeight * getProportion(TIME_CONSUMING_WEIGHT_PROPORTION) /
              WEIGHT_PERCENT_UNIT +
          timeoutRateWeight * getProportion(TIMEOUT_WEIGHT_PROPORTION) /
              WEIGHT_PERCENT_UNIT;
...
void LoadBalanceThread::printLoadCache(LoadCache &loadCache) {
  ...
  for (auto &loadPair : loadCache) {
    ...
    for (auto &loadInfo : loadPair.second.vtBalanceItem) {
      log << loadInfo.statTime << ", " << loadInfo.date << ", "
          << loadInfo.slaveName << ", " << loadInfo.slaveIp << ", "
          << loadInfo.succCount << ", " << loadInfo.timeoutCount << ", "
          << loadInfo.aveTime << ", " << loadInfo.aveCount << ", "
          << loadInfo.port << ", " << loadInfo.weight << ", "
          << loadInfo.weightType << "; ";
    }
