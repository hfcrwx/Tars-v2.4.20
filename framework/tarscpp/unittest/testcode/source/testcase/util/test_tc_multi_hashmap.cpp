...
void set(const string &mk, const string &uk, const string &v, uint8_t version,
         bool dirty, bool full, bool head) {
  vector<TC_Multi_HashMap::Value> vRecords;

  TC_Multi_HashMap::DATATYPE eType =
      (full ? TC_Multi_HashMap::FULL_DATA : TC_Multi_HashMap::PART_DATA);
  ...
  if (tv_2.tv_usec < tv_1.tv_usec) {
    ...
    lTime =
        (tv_2.tv_sec - tv_1.tv_sec) * 1000000 + (tv_2.tv_usec - tv_1.tv_usec);
  }

  // assert(ret == 0);
  cout << "set:" << mk << "|" << uk << "|" << v << "|" << ret << "|" << lTime
       << endl;
...
void get(const string &mk, const string &uk) {
  ...
  if (!uk.empty()) {
    ...
    cout << "get:" << mk << "|" << uk << "|" << v._data._value << "|"
         << (int)v._data._iVersion << "|" << v._data._dirty << "|" << ret
         << endl;
  } else {
    // ����key�µ���������
...
void del(const string &mk, const string &uk) {
  ...
  for (size_t i = 0; i < vtDel.size(); i++) {
    cout << "del:" << vtDel[i]._mkey << "|" << vtDel[i]._data._key << "|"
         << vtDel[i]._data._value << endl;
  }
}
...
void listh() {
  ...
  while (it != g_hmap.hashEnd()) {
    ...
    for (size_t i = 0; i < vv.size(); i++) {
      cout << vv[i]._mkey << "|" << vv[i]._data._key << "|"
           << vv[i]._data._value << "|" << (int)vv[i]._data._iVersion << "|"
           << vv[i]._data._dirty << "|" << vv[i]._data._synct << endl;
    }
  }
}
...
void list(int type, bool asc = true) {
  ...
  while (it != g_hmap.end()) {
    ...
    cout << v._mkey << "|" << v._data._key << "|" << v._data._value << "|"
         << (int)v._data._iVersion << "|" << v._data._dirty << "|"
         << v._data._synct << endl;
  }
}
...
void batchset(size_t iCount, size_t iAvgRec) {
  ...
  for (size_t i = 1; i <= iCount / iAvgRec; i++) {
    ...
    for (size_t j = 1; j <= iRec; j++) {
      ...
      vRecords.clear();
      int ret = g_hmap.set(mk, uk, v, 0, true, TC_Multi_HashMap::FULL_DATA,
                           true, vRecords);
      ...
      if (!g_bSilent) {
        ...
        if (tv_2.tv_usec < tv_1.tv_usec) {
          ...
          lTime = (tv_2.tv_sec - tv_1.tv_sec) * 1000000 +
                  (tv_2.tv_usec - tv_1.tv_usec);
        }

        // assert(ret == 0);
        cout << "set:" << mk << "|" << uk << "|" << v << "|" << ret << "|"
             << lTime << endl;
        for (size_t i = 0; i < vRecords.size(); i++) {
          cout << "set erase:" << vRecords[i]._mkey << "|"
               << vRecords[i]._data._key << "|" << vRecords[i]._data._value
               << endl;
        }
      }
    }
...
void batchset() {
  ...
  int ret =
      g_hmap.set(vtSet, TC_Multi_HashMap::FULL_DATA, true, true, vtErased);

  for (size_t i = 0; i < vtSet.size(); i++) {
    cout << "set:" << vtSet[i]._mkey << "|" << vtSet[i]._data._key << "|"
         << vtSet[i]._data._value << "|" << ret << endl;
  }
  for (size_t i = 0; i < vtErased.size(); i++) {
...
void testMemSize() {
  ...
  cout << "sizeof(tagBlockHead)="
       << sizeof(TC_Multi_HashMap::Block::tagBlockHead) << endl;
  cout << "sizeof(tagMainKeyHead)="
       << sizeof(TC_Multi_HashMap::MainKey::tagMainKeyHead) << endl;
  cout << "sizeof(tagChunkHead)="
       << sizeof(TC_Multi_HashMap::Block::tagChunkHead) << endl;
  cout << "sizeof(tagHashItem)=" << sizeof(TC_Multi_HashMap::tagHashItem)
       << endl;
  cout << "sizeof(tagMainKeyHashItem)="
       << sizeof(TC_Multi_HashMap::tagMainKeyHashItem) << endl;
}
...
void usage(char *argv) {
  ...
  cout << "\t --set [--mkey=] [--ukey=] [--value=] [--version] [--clean] "
          "[--full] [--tail]"
       << endl;
  ...
  cout << "\t --set-batch [--count] [--avgrec]\tdo batch set operation, will "
          "set total count records with avgrec records per main key"
       << endl;
  cout << "\t --get-batch [--count] [--avgrec]\tdo batch get operation" << endl;
  cout << "\t --chkmk [--mkey]\tcheck if main key exists" << endl;
  cout << "\t --setstate [--mkey] [--ukey] [--dirty] [--clean] [--part] "
          "[--full]\tcheck if main key exists"
       << endl;
...
int main_test(int argc, char *argv[]) {
  try {
    ...
    if (option.hasParam("view")) {
      ...
      uint8_t version = option.getValue("version").empty()
                            ? 0
                            : atoi(option.getValue("version").c_str());
      ...
      cout << "isdirty:" << mk << "|" << uk << "|"
           << (ret == TC_Multi_HashMap::RT_DIRTY_DATA) << endl;
      ...
      if (!sCount.empty()) {
        batchset(TC_Common::strto<size_t>(sCount),
                 sAvgRec.empty() ? 2 : TC_Common::strto<size_t>(sAvgRec));
      } else {
        // ������������������
        batchset();
      }
