...
void testSendRand() {
  ...
  for (int i = 0; i < 10000; i++) {
    ...
    // cout << "set:" << v[index].first << ":" << v[index].second << ":" <<
    // g_hmap.size() << ":" << g_hmap.dirtyCount() << endl;
...
void testGetRand() {
  ...
  for (int i = 0; i < 200000; i++) {
    ...
    assert(ret == 0);
    cout << "get:" << v[index].first << ":" << v[index].second << ":"
         << g_hmap.size() << ":" << g_hmap.dirtyCount() << endl;
  }
  ...
  */
  ...
  if (option.hasParam("set-rand")) {
    ...
    for (size_t i = 0; i < vRecords.size(); i++) {
      cout << "set-taf erase:" << vRecords[i]._key << ":" << vRecords[i]._value
           << endl;
    }
