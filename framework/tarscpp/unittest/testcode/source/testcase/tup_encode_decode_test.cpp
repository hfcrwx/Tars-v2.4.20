...
TEST(TupUniPacketTest, encode_and_decode_using_UniPacket) {
  try {
    ...
    cout << "type:" << Class<map<Double, map<Float, vector<Bool> > > >::name()
    << endl; cout << "type:" << Class<map<Int64, vector<UserInfo> > >::name() <<
    endl; cout << "type:" << Class<map<Short, FriendInfo> >::name() << endl;
    */
    ...
    */
    ...
    for (size_t j = 0; j < vinfo.size(); ++j) {
      ...
      for (int i = 0; i < info.birthday.size(); ++i) {
        ...
        snprintf(szBirthday + i, sizeof(szBirthday) - i, "%d",
                 info.birthday[i] - '0');
      }
      if (0 == j) {
    ...
    int index = 0;
    for (map<short, FriendInfo>::iterator it = minfo.begin(); it != minfo.end();
         ++it, ++index) {
      ...
      for (size_t i = 0; i < it->second.birthday.size(); ++i) {
        ...
        snprintf(szBirthday + i, sizeof(szBirthday) - i, "%c",
                 it->second.birthday[i]);
      }
      cout << endl;
    ...
    cout << "---------------------------------" << endl;
    ////////////////////////////////////////
    cout << "{default}:" << server.getByDefault<string>("default", "kaka")
         << endl;
  } catch (exception &e) {
