...
bool TC_URL::isValid() const { return !_sURL.empty(); }

string TC_URL::getURL() const { return _sURL; }
...
bool TC_URL::isDefaultPort() const { return _sPort == getDefaultPort(); }
...
string TC_URL::getScheme() const { return _sScheme; }

string TC_URL::getUserName() const { return _sUser; }

string TC_URL::getPassword() const { return _sPass; }

string TC_URL::getDomain() const { return _sDomain; }

string TC_URL::getPort() const { return _sPort; }

string TC_URL::getPath() const { return _sPath; }

string TC_URL::getQuery() const { return _sQuery; }

string TC_URL::getRef() const { return _sRef; }
...
string TC_URL::simplePath(const string &sPath) const {
  ...
  //如果路径是以.结尾的, 则.去掉
  if (((sNewPath.length() >= 2) &&
       (sNewPath.substr(sNewPath.length() - 2) == "/.")) ||
      (sNewPath == ".")) {
  ...
  //处理/..结尾的情况
  if ((sNewPath.size() >= 3) &&
      (sNewPath.substr(sNewPath.size() - 3, 3) == "/..")) {
...
string TC_Http::getContentType() const { return getHeader("Content-Type"); }

string TC_Http::getHost() const { return getHeader("Host"); }
...
string TC_Http::genHeader() const {
  ...
  for (http_header_type::const_iterator it = _headers.begin();
       it != _headers.end(); ++it) {
    if (it->second != "") {
...
vector<string> TC_Http::getHeaderMulti(const string &sHeadName) const {
  ...
  for (http_header_type::const_iterator it = _headers.begin(); it != itEnd;
       ++it) {
...
bool TC_HttpCookie::matchDomain(const string &sCookieDomain,
  ...
  if (sDomain.length() >= sCookieDomainNew.length() &&
      sLowerDomain.compare(sDomain.length() - sCookieDomainNew.length(),
                           sCookieDomainNew.length(), sCookieDomainNew) == 0) {
    return true;
  }
...
size_t TC_HttpCookie::matchPath(const string &sCookiePath,
                                const string &sPath) {
  if (sCookiePath.empty() || sPath.empty()) return 0;

  //都在最后加/再匹配
  string sCookiePath1 = (sCookiePath.at(sCookiePath.length() - 1) == '/')
                            ? sCookiePath
                            : sCookiePath + "/";
...
void TC_HttpCookie::clear() { _cookies.clear(); }
...
void TC_HttpCookie::addCookie(const Cookie &cookie, list<Cookie> &cookies) {
  ...
  while (it != cookies.end()) {
    ...
    if (isCookieExpires(*it)) {
      cookies.erase(it++);
    } else if (TC_Port::strcasecmp(it->_domain.c_str(),
                                   cookieNew._domain.c_str()) == 0 &&
...
void TC_HttpCookie::addCookie(const string &sRspURL,
                              const vector<string> &vCookies) {
  TC_URL cURL;
  ...
  for (size_t i = 0; i < vCookies.size(); i++) {
    ...
    for (size_t j = 0; j < v.size(); ++j) {
      ...
      if (TC_Port::strcasecmp(name.c_str(), "secure") == 0) {
        ...
        if (value.length() == 27 && value.at(11) == ' ' &&
            value.at(14) == ' ') {
...
size_t TC_HttpCookie::isCookieMatch(const Cookie &cookie,
                                    const TC_URL &tURL) const {
  //域名没有匹配
  ...
  //安全的cookie,不安全的URL
  if (cookie._isSecure && (tURL.getType() != TC_URL::HTTPS)) return 0;

  return len;
}

void TC_HttpCookie::getCookieForURL(const string &sReqURL,
                                    list<TC_HttpCookie::Cookie> &cookies) {
  TC_URL tURL;
...
void TC_HttpCookie::getCookieForURL(const string &sReqURL, string &sCookie) {
  ...
  while (it != cookies.end()) {
    ...
    while (itd != it->_data.end()) {
      //被删除的cookie不输出
      if (itd->first != "" && itd->second != "" &&
          TC_Common::lower(itd->second) != "null" &&
...
size_t TC_HttpResponse::parseResponseHeaderString(const char *beginIt,
                                                  const char *headerIt) {
  ...
  if (f1 == NULL) {
    throw TC_HttpResponse_Exception(
        "[TC_HttpResponse_Exception::parseResponeHeader] http response parse "
  ...
  if (TC_Port::strncasecmp(_headerLine.c_str(), "HTTP/", 5) != 0) {
    throw TC_HttpResponse_Exception(
        "[TC_HttpResponse_Exception::parseResponeHeader] http response version "
...
bool TC_HttpResponse::incrementDecode(TC_NetWorkBuffer &buff) {
  ...
  if (_headLength == 0) {
    ...
    _iTmpContentLength =
        parseResponseHeaderString(data.first, data.first + _headLength);
  ...
  if (_bIsChunked) {
    while (true) {
      ...
      auto sit = std::search(buff.begin(), buff.end(), sep.c_str(),
                             sep.c_str() + sep.size());
...
bool TC_HttpResponse::decode(const char *sBuffer, size_t iLength) {
  ...
  // body内容长度为0或者没有content-length  且 非chunk模式, 则认为包收全了,
  // 直接返回
...
void TC_HttpResponse::setResponse(int status, const string &about,
                                  const string &body) {
  _status = status;
  _about = about;
  _content = body;
...
void TC_HttpResponse::setResponse(int status, const string &about,
                                  const char *sBuffer, size_t iLength) {
  _status = status;
  _about = about;
...
void TC_HttpRequest::setRequest(const string &method, const string &sUrl,
                                const std::string &body, bool bNewCreateHost) {
...
void TC_HttpRequest::setPostRequest(const string &sUrl, const string &sPostBody,
                                    bool bNewCreateHost) {
  if (bNewCreateHost) {
    eraseHeader("Host");
  }
...
void TC_HttpRequest::setOptionsRequest(const string &sUrl,
                                       bool bNewCreateHost) {
  if (bNewCreateHost) {
    eraseHeader("Host");
  }
...
void TC_HttpRequest::setPostRequest(const string &sUrl, const char *sBuffer,
                                    size_t iLength, bool bNewCreateHost) {
  assert(sBuffer != NULL);
...
void TC_HttpRequest::setPutRequest(const string &sUrl, const string &sPostBody,
                                   bool bNewCreateHost) {
  if (bNewCreateHost) {
    eraseHeader("Host");
  }
...
void TC_HttpRequest::setPatchRequest(const string &sUrl,
                                     const string &sPostBody,
                                     bool bNewCreateHost) {
  if (bNewCreateHost) {
    eraseHeader("Host");
  }
...
void TC_HttpRequest::setDeleteRequest(const string &sUrl,
                                      const string &sPostBody,
                                      bool bNewCreateHost) {
  if (bNewCreateHost) {
    eraseHeader("Host");
...
string TC_HttpRequest::encode() {
  string sRet;
  sRet.reserve(128);
  if (_requestType == REQUEST_GET || _requestType == REQUEST_OPTIONS ||
      _requestType == REQUEST_HEAD) {
    ...
    genHeader(sRet);
    sRet += "\r\n";
  } else if (_requestType == REQUEST_POST || _requestType == REQUEST_PUT ||
             _requestType == REQUEST_PATCH || _requestType == REQUEST_DELETE) {
...
bool TC_HttpRequest::decode(const char *sBuffer, size_t iLength) {
  ...
  bool bChunk =
      checkHeader("Transfer-Encoding",
                  "chunked");  //(getHeader("Transfer-Encoding") == "chunked");
  ...
  if (bChunk) {
    ...
    while (true) {
      ...
      if ((sBuffer + iLength - p) >=
          (pos - p) + 2 + iChunkSize + 2)  //接收到一个完整的chunk了
      {
        //获取一个chunk的内容
...
bool TC_HttpRequest::checkRequest(const char *sBuffer, size_t iLen) {
  ...
  if (p == NULL) {
    throw runtime_error(
        "[TC_HttpRequest::checkRequest] http protocol parse error");
  }

  auto it = TC_Http::HEADER.find(string(sBuffer, p - sBuffer));
  if (it == TC_Http::HEADER.end()) {
    throw runtime_error(
        "[TC_HttpRequest::checkRequest] protocol not support: " +
        string(sBuffer, 8));
  }
  ...
  while (true) {
    ...
    if (lineEnd != NULL) {
      ...
      if (TC_Port::strncasecmp(line, "Transfer-Encoding:", 18) == 0) {
        ...
        len = TC_Common::strto<size_t>(
            TC_Common::trim(string(line + 15, lineEnd), " "));
      }
    }
  }
...
size_t TC_Http::parseHeaderString(const char *beginIt, const char *headerIt,
                                  TC_Http::http_header_type &sHeader) {
  ...
  while (true) {
    ...
    if (!first) {
      ...
      if (itF != NULL) {
        ...
        sHeader.insert(multimap<string, string>::value_type(std::move(name),
                                                            std::move(value)));
      }
    } else {
      first = false;
...
void TC_HttpRequest::parseRequestHeader(const char *szBuffer,
                                        const char *header) {
  ...
  if (f1 == p) {
    throw TC_HttpRequest_Exception(
        "[TC_HttpRequest::parseRequestHeader] http request format error: " +
  ...
  auto f2 = strnstr(
      f1 + 1, " ",
      p - f1 +
          1);  // std::search(f1 + 1, p, sep.c_str(), sep.c_str() + sep.size());
  if (f2 == p || f1 >= f2) {
    throw TC_HttpRequest_Exception(
        "[TC_HttpRequest::parseRequestHeader] http request format error: " +
        string(szBuffer, p - szBuffer));
...
const string &TC_HttpRequest::getMethod() const {
  ...
  if (it != TC_Http::HEADER_REVERSE.end()) {
    ...
    throw TC_HttpRequest_Exception(
        "[TC_HttpRequest::setMethod] http request command error: " +
        TC_Common::tostr(_requestType));
  }
}
...
void TC_HttpRequest::setPath(const char *path) { _httpURL._sPath = path; }
...
int TC_HttpRequest::doRequest(TC_TCPClient &tcpClient,
