...
void oi_symmetry_encrypt2(const char *pInBuf, int nInBufLen, const char *pKey,
                          char *pOutBuf, size_t *pOutBufLen);
bool oi_symmetry_decrypt2(const char *pInBuf, int nInBufLen, const char *pKey,
                          char *pOutBuf, size_t *pOutBufLen);

void TC_Tea::encrypt(const char *key, const char *sIn, size_t iLength,
                     vector<char> &buffer) {
...
bool TC_Tea::decrypt(const char *key, const char *sIn, size_t iLength,
                     vector<char> &buffer) {
...
*/
/*TEA加密算法,CBC模式*/
/*密文格式:PadLen(1byte)+Padding(var,0-7byte)+Salt(2byte)+Body(var
 * byte)+Zero(7byte)*/
int oi_symmetry_encrypt2_len(int nInBufLen) {
  ...
  nPadSaltBodyZeroLen = nInBufLen /*Body长度*/ + 1 + SALT_LEN +
                        ZERO_LEN /*PadLen(1byte)+Salt(2byte)+Zero(7byte)*/;
...
void oi_symmetry_encrypt2(const char *pInBuf, int nInBufLen, const char *pKey,
  ...
  src_buf[0] =
      (((char)rand()) & 0x0f8) /*最低三位存PadLen,清零*/ | (char)nPadlen;
...
/*
   ...
   *pOutBufLen为接收缓冲区的长度 特别注意*pOutBufLen应预置接收缓冲区的长度!
...
bool oi_symmetry_decrypt2(const char *pInBuf, int nInBufLen, const char *pKey,
  ...
  /*密文格式:PadLen(1byte)+Padding(var,0-7byte)+Salt(2byte)+Body(var
   * byte)+Zero(7byte)*/
  i = nInBufLen - 1 /*PadLen(1byte)*/ - nPadLen - SALT_LEN -
      ZERO_LEN; /*明文长度*/
