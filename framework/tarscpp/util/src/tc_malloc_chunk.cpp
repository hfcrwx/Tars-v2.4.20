...
void SizeMap::Init() {
  ...
  //对于32K一页的内存，
  // 1.
  // 首先，对一页内存块按某一内存大小iSize进行分割，（iSize的选取，从大小为8开始，然后按8，16，32，64，128，。。。增量进行选取）
  //如果分割后浪费的内存大于4k，则增加一页，同时设定其浪费的内存大小为2*4k，
  //如果分离后浪费的内存大小大于2*4k，则再增加一页内存，同时设定其浪费的内存大小为3*4k，
  //以此执行，直到浪费的内存大小小于等于n*4k；
  // 2.
  // 在保证条件1满足的情况下，对于n个内存页，按条件1中的iSize大小分割而得到的个数num，进行判断
  ...
  for (size_t size = kAlignment; size <= kMaxSize; size += alignment) {
    ...
    if (sc > 1 && my_pages == class_to_pages_[sc - 1]) {
      ...
      const size_t prev_objects =
          (class_to_pages_[sc - 1] << kPageShift) / class_to_size_[sc - 1];
...
void TC_SpanAllocator::create(void *pAddr, size_t iSpanSize,
                              size_t iSpanCount) {
...
void TC_SpanAllocator::connect(void *pAddr) { init(pAddr); }

void *TC_SpanAllocator::allocate() {
  if (!isSpanAvailable()) return NULL;

  unsigned char *result =
      _pData + (_pHead->_firstAvailableSpan * _pHead->_iSpanSize);
...
void TC_SpanAllocator::deallocate(void *pAddr) {
  ...
  *((size_t *)prelease) = _pHead->_firstAvailableSpan;
  ...
  _pHead->_firstAvailableSpan =
      static_cast<size_t>((prelease - _pData) / _pHead->_iSpanSize);

  assert(_pHead->_firstAvailableSpan ==
         (prelease - _pData) / _pHead->_iSpanSize);
...
void TC_Page::DLL_Remove(TC_Span *span) {
  TC_Span *_prev = reinterpret_cast<TC_Span *>(
      reinterpret_cast<size_t>(_pShmFlagHead) + span->prev);
  TC_Span *_next = reinterpret_cast<TC_Span *>(
      reinterpret_cast<size_t>(_pShmFlagHead) + span->next);
...
TC_Page::TC_Span *TC_Page::SearchFreeAndLargeLists(size_t n) {
  for (size_t s = n; s < kMaxPages; ++s) {
    TC_Span *ll = &_pFree[s];
    if (!DLL_IsEmpty(ll, reinterpret_cast<size_t>(ll) -
                             reinterpret_cast<size_t>(_pShmFlagHead))) {
      return Carve(reinterpret_cast<TC_Span *>(
                       reinterpret_cast<size_t>(_pShmFlagHead) + ll->next),
                   n);
    }
  }
  return AllocLarge(n);
...
TC_Page::TC_Span *TC_Page::AllocLarge(size_t n) {
  ...
  for (; span != _pLarge;
       span = reinterpret_cast<TC_Span *>(iBeginAddr + span->next)) {
    if (span->length >= n) {
...
TC_Page::TC_Span *TC_Page::Carve(TC_Span *span, size_t n) {
  ...
  if (extra >= kMaxPages) {
    ...
    if (flag) {
      ...
      update(
          (void *)(&(leftover->location)),
          static_cast<unsigned int>(
              TC_Span::ON_FREELIST));  // update((void*)(&(leftover->location)),
                                       // TC_Span::IN_USE);

      update((void *)(&(_pPageMap[span->start])),
             reinterpret_cast<size_t>(leftover) -
                 reinterpret_cast<size_t>(_spanAlloc.getHead()));
      if (n > 1) {
        update((void *)(&(_pPageMap[span->start + n - 1])),
               reinterpret_cast<size_t>(leftover) -
                   reinterpret_cast<size_t>(_spanAlloc.getHead()));
      }
      ...
      update((void *)(&(_pPageMap[span->start + n])),
             reinterpret_cast<size_t>(span) -
                 reinterpret_cast<size_t>(_spanAlloc.getHead()));
      if (extra > 1) {
        update((void *)(&(_pPageMap[span->start + n + extra - 1])),
               reinterpret_cast<size_t>(span) -
                   reinterpret_cast<size_t>(_spanAlloc.getHead()));
      }
      doUpdate(true);
    ...
    if (flag) {
      ...
      update((void *)(&(span->location)),
             static_cast<unsigned int>(TC_Span::ON_FREELIST));
    } else {
      _prev->next = span->next;
      _next->prev = span->prev;
      span->prev = 0;
    ...
    if (extra > 0) {
      ...
      if (flag) {
        ...
        update((void *)(&(leftover->prev)),
               reinterpret_cast<size_t>(list) - iBeginAddr);
        update((void *)(&(_pnext->prev)),
               reinterpret_cast<size_t>(leftover) - iBeginAddr);
        update((void *)(&(list->next)),
               reinterpret_cast<size_t>(leftover) - iBeginAddr);
    ...
    if (p > 0) {
      ...
      if (pPrev != NULL && pPrev->location == location) {
        ...
        if (len >= kMaxPages) {
          ...
          update((void *)(&(_pPageMap[p + n - 1])),
                 reinterpret_cast<size_t>(pPrev) - spanBeginAddr);
          doUpdate(true);
          ...
          if (pPrev->prev != 0 && pPrev->next != 0) {
            TC_Span *_prev =
                reinterpret_cast<TC_Span *>(iBeginAddr + pPrev->prev);
            TC_Span *_next =
                reinterpret_cast<TC_Span *>(iBeginAddr + pPrev->next);
          ...
          update((void *)(&(_pPageMap[p])),
                 reinterpret_cast<size_t>(span) - spanBeginAddr);
          doUpdate(true);
    ...
    if (p + n < _pShmFlagHead->_iShmPageMapNum) {
      ...
      if (pNext != NULL && pNext->location == location) {
        ...
        if (n >= kMaxPages && len >= kMaxPages) {
          if (prevMaxFlag) {
            TC_Span *_prev =
                reinterpret_cast<TC_Span *>(iBeginAddr + span->prev);
            TC_Span *_next =
                reinterpret_cast<TC_Span *>(iBeginAddr + span->next);
            ...
            update((void *)(&(_pPageMap[p])),
                   reinterpret_cast<size_t>(pNext) - spanBeginAddr);
          ...
          if (prevMaxFlag) {
            if (pNext->prev != 0 && pNext->next != 0) {
              TC_Span *_prev =
                  reinterpret_cast<TC_Span *>(iBeginAddr + pNext->prev);
              TC_Span *_next =
                  reinterpret_cast<TC_Span *>(iBeginAddr + pNext->next);
            ...
            TC_Span *_pnext =
                reinterpret_cast<TC_Span *>(iBeginAddr + list->next);

            update((void *)(&(span->next)), list->next);
            update((void *)(&(span->prev)),
                   reinterpret_cast<size_t>(list) - iBeginAddr);
            update((void *)(&(_pnext->prev)),
                   reinterpret_cast<size_t>(span) - iBeginAddr);
            update((void *)(&(list->next)),
                   reinterpret_cast<size_t>(span) - iBeginAddr);
...
void TC_Page::MergeIntoFreeList(TC_Span *span) {
  ...
  if (p > 0) {
    ...
    if (pPrev != NULL && pPrev->location == span->location) {
      ...
      span->start -= len;
      span->length += len;
      Set(span->start, reinterpret_cast<size_t>(span) -
                           reinterpret_cast<size_t>(_spanAlloc.getHead()));
    }
  }
  if (p + n < _pShmFlagHead->_iShmPageMapNum) {
...
void TC_Page::RegisterSizeClass(TC_Span *span, size_t sc) {
  ...
  for (size_t i = 1; i < span->length - 1; ++i) {
    Set(span->start + i, reinterpret_cast<size_t>(span) -
                             reinterpret_cast<size_t>(_spanAlloc.getHead()));
  }
}
...
TC_Page::TC_Span *TC_Page::GetDescriptor(size_t pPageId) {
  ...
  if (pPageId < end) {
    return reinterpret_cast<TC_Span *>(
        reinterpret_cast<char *>(_spanAlloc.getHead()) + Get(pPageId));
  } else {
    return NULL;
  }
}
...
bool TC_Page::UseShmMem() {
  ...
  const size_t p =
      (ptr - iBeginAddr - _pShmFlagHead->_iShmPageAddr) >> kPageShift;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void *TC_Page::fetchFromSpansSafe(size_t iClassSize, size_t &iAllocSize,
                                  size_t &iPageId, size_t &iIndex) {
...
void *TC_Page::FetchFromSpans(size_t iClassSize, size_t &iAllocSize,
                              size_t &iPageId, size_t &iIndex) {
  size_t iBeginAddr = reinterpret_cast<size_t>(_pShmFlagHead);
  if (DLL_IsEmpty(
          &_pCenterCache[iClassSize].nonempty,
          reinterpret_cast<size_t>(&_pCenterCache[iClassSize].nonempty) -
              iBeginAddr))
    return NULL;

  TC_Span *span = reinterpret_cast<TC_Span *>(
      iBeginAddr + _pCenterCache[iClassSize].nonempty.next);

  void *result = reinterpret_cast<void *>(iBeginAddr + span->objects);
  size_t _size = Static::sizemap()->ByteSizeForClass(iClassSize);
  iPageId = span->start;
  iIndex = (span->objects -
            (_pShmFlagHead->_iShmPageAddr + (iPageId << kPageShift))) /
           _size;
...
int TC_Page::Populate(size_t iClassSize) {
  ...
  if (span) {
    if (flag) {
      update((void *)(&(span->location)),
             static_cast<unsigned int>(TC_Span::IN_USE));
      update((void *)(&(span->sizeclass)),
             static_cast<unsigned int>(iClassSize));
      for (size_t i = 1; i < span->length - 1; ++i) {
        update((void *)(&(_pPageMap[span->start + i])),
               reinterpret_cast<size_t>(span) -
                   reinterpret_cast<size_t>(_spanAlloc.getHead()));
      }
    } else {
  ...
  char *ptr = reinterpret_cast<char *>(
      iBeginAddr + _pShmFlagHead->_iShmPageAddr + (span->start << kPageShift));
...
void TC_Page::releaseToSpans(size_t iPageId, size_t iIndex) {
  ...
  if (ref == 0) {
    ...
    void *ptr = reinterpret_cast<void *>(
        iBeginAddr + _pShmFlagHead->_iShmPageAddr + (iPageId << kPageShift) +
        iIndex * Static::sizemap()->ByteSizeForClass(_size_class));
    if (flag) {
      update(ptr, span->objects);
      update((void *)(&(span->objects)),
             reinterpret_cast<size_t>(ptr) - iBeginAddr);
      update((void *)(&(span->refcount)), ref);
    } else {
...
void TC_Page::releaseToSpans(void *pObject) {
  size_t iBeginAddr = reinterpret_cast<size_t>(_pShmFlagHead);
  const size_t _page_id = (reinterpret_cast<size_t>(pObject) -
                           (iBeginAddr + _pShmFlagHead->_iShmPageAddr)) >>
                          kPageShift;
  ...
  if (ref == 0) {
    ...
    if (flag) {
      ...
      update((void *)(&(span->objects)),
             reinterpret_cast<size_t>(pObject) - iBeginAddr);
      update((void *)(&(span->refcount)), ref);
    } else {
...
void *TC_Page::getAbsolute(size_t iPageId, size_t iIndex) {
  ...
  return reinterpret_cast<void *>(
      reinterpret_cast<size_t>(_pShmFlagHead) + _pShmFlagHead->_iShmPageAddr +
      (iPageId << kPageShift) +
      iIndex * Static::sizemap()->ByteSizeForClass(_size_class));
}
...
void TC_Page::doUpdate(bool bUpdate) {
  ...
  if (_pModifyHead->_cModifyStatus == 1) {
    ...
    for (size_t i = 0;
         i < sizeof(_pModifyHead->_stModifyData) / sizeof(tagModifyData); i++) {
    ...
    for (size_t i = 0; i < _pModifyHead->_iNowIndex; i++) {
      if (_pModifyHead->_stModifyData[i]._cBytes == sizeof(size_t)) {
        *(reinterpret_cast<size_t *>(
            reinterpret_cast<size_t>(_pShmFlagHead) +
            _pModifyHead->_stModifyData[i]._iModifyAddr)) =
            _pModifyHead->_stModifyData[i]._iModifyValue;
      } else if (_pModifyHead->_stModifyData[i]._cBytes ==
                 sizeof(unsigned int)) {
...
void TC_Page::init(void *pAddr) {
  ...
  _pModifyHead = static_cast<tagModifyHead *>(
      (void *)((char *)_pShmFlagHead + sizeof(tagShmFlag)));

  _pCenterCache = static_cast<TC_CenterList *>(
      (void *)((char *)_pModifyHead + sizeof(tagModifyHead)));

  _pLarge = static_cast<TC_Span *>(
      (void *)((char *)_pCenterCache + sizeof(TC_CenterList) * kNumClasses));
...
void TC_Page::initPage(void *pAddr) {
  void *span_begin_ptr =
      static_cast<void *>((char *)_pFree + sizeof(TC_Span) * kMaxPages);
  size_t begin = reinterpret_cast<size_t>(pAddr);
  size_t end = reinterpret_cast<size_t>(span_begin_ptr);

  _pSpanMemHead =
      static_cast<TC_SpanAllocator *>((void *)((char *)span_begin_ptr));
  _pShmFlagHead->_iShmSpanAddr = end - begin;

  size_t _iCapacitySize = _pShmFlagHead->_iShmMemSize - (end - begin);
  size_t _iPagesNum = (_iCapacitySize - TC_SpanAllocator::getHeadSize() -
                       sizeof(TC_Span) * 20) /
                      (sizeof(TC_Span) + sizeof(size_t) + kPageSize);

  size_t *pagemap_ptr = reinterpret_cast<size_t *>(
      (char *)_pSpanMemHead + TC_SpanAllocator::getHeadSize() +
      sizeof(TC_Span) * (_iPagesNum + 20));
  end = reinterpret_cast<size_t>(pagemap_ptr);
  _pPageMap = pagemap_ptr;
  _pShmFlagHead->_iShmPageMapAddr = end - begin;

  void *data_ptr =
      reinterpret_cast<void *>((char *)_pPageMap + sizeof(size_t) * _iPagesNum);
  end = reinterpret_cast<size_t>(data_ptr);
  _pData = data_ptr;
  _pShmFlagHead->_iShmPageAddr = end - begin;

  memset(_pPageMap, 0, sizeof(size_t) * _iPagesNum);
  _spanAlloc.create((void *)((char *)_pSpanMemHead), sizeof(TC_Span),
                    (_iPagesNum + 20));
...
void TC_Page::create(void *pAddr, size_t iShmMemSize, bool bProtectedArea) {
  ...
  for (size_t i = 0; i < kNumClasses; ++i) {
    ...
    DLL_Init(&_pCenterCache[i].empty,
             reinterpret_cast<size_t>(&_pCenterCache[i].empty) - iBeginAddr);
    DLL_Init(&_pCenterCache[i].nonempty,
             reinterpret_cast<size_t>(&_pCenterCache[i].nonempty) - iBeginAddr);
  }
...
void TC_Page::connect(void *pAddr) {
  ...
  _pSpanMemHead = reinterpret_cast<TC_SpanAllocator *>(
      iBeginAddr + _pShmFlagHead->_iShmSpanAddr);
  _spanAlloc.connect((void *)((char *)_pSpanMemHead));
  _pPageMap =
      reinterpret_cast<size_t *>(iBeginAddr + _pShmFlagHead->_iShmPageMapAddr);
...
size_t TC_Page::getPageNumber() { return _pShmFlagHead->_iShmPageNum; }
...
void TC_MallocChunkAllocator::create(void *pAddr, size_t iSize,
                                     bool bProtectedArea) {
...
void TC_MallocChunkAllocator::_connect(void *pAddr) {
  ...
  tagChunkAllocatorHead *pNextHead =
      (tagChunkAllocatorHead *)((char *)_pHead + _pHead->_iNext);
...
TC_MallocChunkAllocator *TC_MallocChunkAllocator::lastAlloc() {
  if (_nallocator == NULL) return NULL;
...
size_t TC_MallocChunkAllocator::getAllCapacity() {
  if (_nallocator == NULL) return _page.getPageMemSize();
