...
const std::vector<std::string> TC_Cron::DAYS = {"SUN", "MON", "TUE", "WED",
                                                "THU", "FRI", "SAT"};
const std::vector<std::string> TC_Cron::MONTHS = {
...
TC_Cron TC_Cron::makecron(const string& expr) {
  ...
  if (fields.size() != 6)
    throw TC_CronException(
        "[TC_Cron makecron] cron expression must have six fields");
...
std::time_t TC_Cron::tm_to_time(std::tm& tmt) { return std::mktime(&tmt); }
...
std::string TC_Cron::replaceOrdinals(
...
std::pair<cron_int, cron_int> TC_Cron::makeRange(std::string field,
                                                 cron_int minval,
                                                 cron_int maxval) {
  cron_int first = 0;
  cron_int last = 0;
  if (field.size() == 1 && field[0] == '*') {
    ...
    if (parts.size() != 2)
      throw TC_CronException(
          "TC_Cron::makeRange] specified range requires two fields");
  ...
  if (first > maxval || last > maxval) {
    throw TC_CronException(
        "TC_Cron::makeRange] specified range exceeds maximum");
  }
  if (first < minval || last < minval) {
    throw TC_CronException(
        "TC_Cron::makeRange] specified range is less than minimum");
  }
  if (first > last) {
    throw TC_CronException(
        "TC_Cron::makeRange] specified range start exceeds range end");
  }
...
void TC_Cron::setCronDaysOfWeek(std::string value, std::bitset<7>& target) {
  ...
  setCronField(days_replaced, target, CRON_MIN_DAYS_OF_WEEK,
...
void TC_Cron::resetAllFields(std::tm& tmt,
...
size_t TC_Cron::findNextDay(std::tm& tmt, std::bitset<31> const& days_of_month,
                            size_t day_of_month,
...
bool TC_Cron::findNext(const TC_Cron& cron, std::tm& tmt, size_t const dot) {
  ...
  auto updated_second =
      findNext(cron.seconds, tmt, CRON_MIN_SECONDS, CRON_MAX_SECONDS, second,
               CronField::second, CronField::minute, empty_list);
  ...
  auto update_minute =
      findNext(cron.minutes, tmt, CRON_MIN_MINUTES, CRON_MAX_MINUTES, minute,
               CronField::minute, CronField::hour_of_day, marked_fields);
  ...
  auto updated_hour =
      findNext(cron.hours, tmt, CRON_MIN_HOURS, CRON_MAX_HOURS, hour,
               CronField::hour_of_day, CronField::day_of_week, marked_fields);
  ...
  auto updated_day_of_month =
      findNextDay(tmt, cron.days_of_month, day_of_month, cron.days_of_week,
                  day_of_week, marked_fields);
  ...
  auto updated_month =
      findNext(cron.months, tmt, CRON_MIN_MONTHS, CRON_MAX_MONTHS, month,
