...
class XmlValueString : public XmlValue {
 public:
  XmlValueString(const string& s, bool _cdata = false)
      : value(s), cdata(_cdata) {}
...
class BufferXmlReader {
 ...
 public:
  ...
  void check(size_t pos) {
    if (pos > _len) {
      char s[64];
      snprintf(s, sizeof(s), "buffer[%u] overflow when peekBuf, over %u.",
               (uint32_t)pos, (uint32_t)_len);
...
class TC_Xml {
 public:
  ...
  static void writeValue(const XmlValuePtr& p, vector<char>& buf,
                         bool bHead = true);
  // Xml字符串到Xml结构的转换
 ...
 private:
  ...
  static void writeArray(std::ostream& os, const string& name,
                         const XmlValuePtr& p);
