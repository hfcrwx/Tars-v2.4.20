 ...
 * @brief Network buffer resolution, the main purpose is to avoid buffer copy,
 * speed up
 *
 */
...
class TC_NetWorkBuffer {
 public:
  ...
  enum PACKET_TYPE {
    ...
    PACKET_FULL = 1,
    PACKET_FULL_CLOSE =
        2,  ///< get whole package, and need close connection, for example: http
    PACKET_ERR = -1,
  };
   ...
   */
  typedef std::function<PACKET_TYPE(TC_NetWorkBuffer &, vector<char> &)>
      protocol_functor;
  ...
  class Buffer {
   public:
    ...
    Buffer(const char *sBuffer, size_t length)
        : _buffer(sBuffer, sBuffer + length) {}
    ...
    bool empty() const { return _buffer.size() <= _pos; }
    ...
    char &at(size_t offset) {
      if (_pos + offset >= _buffer.size())
        throw TC_NetWorkBuffer_Exception("[TC_NetWorkBuffer::Buffer] at '" +
                                         TC_Common::tostr(offset) +
                                         "' offset overflow");
  ...
  typedef std::list<std::shared_ptr<Buffer>>::const_iterator
      buffer_list_iterator;

  class buffer_iterator
      : public std::iterator<std::random_access_iterator_tag, char> {
   public:
    buffer_iterator(const TC_NetWorkBuffer *buffer, size_t offset)
        : _buffer(buffer) {
      parseOffset(offset);
    }
   ...
   protected:
    void parseOffset(size_t offset) {
      ...
      bool flag = false;
      for (auto it = _buffer->_bufferList.begin();
           it != _buffer->_bufferList.end(); ++it) {
        if (offset >= (*it)->length()) {
          offset -= (*it)->length();
        } else {
          _it = it;
   ...
   * Must be constructed as a connection (different service models get different
   * objects and need to cast themselves)
   * @param buff
   */
   ...
   * Get connection, get different objects in different service models, need to
   * force conversion yourself
   * @param buff
   */
  ...
  void setContextData(void *contextData,
                      std::function<void(TC_NetWorkBuffer *)> deconstruct =
                          std::function<void(TC_NetWorkBuffer *)>()) {
   ...
   * Gets all buffers from the beginning to the iterator (the previous one
   * excluding the iterator itself) T: string or vector<char>
   * @param it
   * @return
   */
  template <typename T>
  T iteratorToIterator(const buffer_iterator &sit, const buffer_iterator &eit) {
    ...
    if (sit > eit) {
      throw TC_NetWorkBuffer_Exception(
          "[TC_NetWorkBuffer::iteratorToIterator] sit > eit error");
    }
   ...
   * @return const char *, Returns a pointer to the first data buffer, or NULL
   * if empty
   */
   ...
   * Return all buffers (stitch all buffers together, pay attention to
   * performance)
   * @return string
   */
  ...
  /**
   * 读取len字节的buffer(避免len个字节被分割到多个buffer的情况)(注意:
   * 不往后移动) Read buffer of len bytes (to avoid splitting len bytes into
   * multiple buffers) (Note: Do not move backwards)
   * @param len
   * @return
   */
   ...
   * 读取len字节的buffer(避免len个字节被分割到多个buffer的情况)(注意:
   * 不往后移动) Read buffer of len bytes (to avoid splitting len bytes into
   * multiple buffers) (Note: Do not move backwards) getHeader<string>(10),
   * getHeader<vector<char>>(10);
   * @param len
  ...
  T getHeader(size_t len) const {
    if (getBufferLength() < len) {
      throw TC_NetWorkBuffer_Exception(
          "[TC_NetWorkBuffer::getHeader] no enough buff(" +
          TC_Common::tostr(getBufferLength()) + ") to get(" +
          TC_Common::tostr(len) + ")");
    }
   ...
   * Take an integer value of two bytes (byte order), and return 0 if the length
   * is less than 1
   * @return int8_t
   */
   ...
   * Take an integer value of four bytes (byte order), and return 0 if the
   ...
   * Parse a package with a 1-byte header and parse out the package body (move
   * back after parsing) 注意: buffer只返回包体, 不包括头部的1个字节的长度 Note:
   * Buffer only returns the package, not including the length of one byte of
   * the head
   * @param buffer, 输出的buffer
   * @param buffer, Output buffer
   * @param minLength, buffer最小长度, 如果小于, 则认为是错误包,
   * 会返回PACKET_ERR
   * @param minLength, minimum buffer length, if less than, is considered an
   * error package and returns PACKET_ERR
   * @param maxLength, buffer最大长度, 如果超过, 则认为是错误包,
   * 会返回PACKET_ERR
   * @param maxLength, maximum buffer length, if exceeded, is considered an
   * error package and returns PACKET_ERR
   * @return PACKET_TYPE
   */
  PACKET_TYPE parseBufferOf1(vector<char> &buffer, uint8_t minLength,
                             uint8_t maxLength);

  /**
   * 解析一个包头是2字节(字节序)的包, 把包体解析出来(解析后, 往后移动)
   * Parse a 2-byte (byte order) packet header and parse the package body (move
   * back after parsing) 注意: buffer只返回包体, 不包括头部的2个字节的长度 Note:
   * Buffer only returns the package, not including the length of 2 bytes of the
   * header
   * @param minLength, buffer最小长度, 如果小于, 则认为是错误包,
   * 会返回PACKET_ERR
   * @param minLength, minimum buffer length, if less than, is considered an
   ...
   * @return PACKET_TYPE
   */
  PACKET_TYPE parseBufferOf2(vector<char> &buffer, uint16_t minLength,
                             uint16_t maxLength);

  /**
   * 解析一个包头是4字节(字节序)的包, 把包体解析出来(解析后, 往后移动)
   * Parse a package whose header is a 4-byte (byte order) package and parse out
   * the package body (move back after parsing) 注意: buffer只返回包体,
   * 不包括头部的4个字节的长度 Note: Buffer only returns the package, not
   * including the length of 4 bytes of the header
   * @param minLength, buffer最小长度, 如果小于, 则认为是错误包,
   * 会返回PACKET_ERR
   * @param minLength, minimum buffer length, if less than, is considered an
   * error package and returns PACKET_ERR
   ...
   * @return PACKET_TYPE
   */
  PACKET_TYPE parseBufferOf4(vector<char> &buffer, uint32_t minLength,
                             uint32_t maxLength);

  /**
   * 解析二进制包, 1字节长度+包体(iMinLength<包长<iMaxLength,
   * 否则返回PACKET_ERR) Parse binary package, 1 byte length + package
   * (iMinLength<package length<iMaxLength, otherwise return PACKET_ERR) 注意:
   * out只返回包体, 不包括头部的1个字节的长度 Note: out only returns the
   * package, not including the length of one byte of the head
   * @param in
   * @param out
   * @return
   */
  template <uint8_t iMinLength, uint8_t iMaxLength>
  static TC_NetWorkBuffer::PACKET_TYPE parseBinary1(TC_NetWorkBuffer &in,
                                                    vector<char> &out) {
    return in.parseBufferOf1(out, iMinLength, iMaxLength);
  }

  /**
   * 解析二进制包, 2字节长度(字节序)+包体(iMinLength<包长<iMaxLength,
   * 否则返回PACKET_ERR) Parse binary package, 2 byte length (byte order) +
   * package (iMinLength<package length<iMaxLength, otherwise return PACKET_ERR)
  ...
  static TC_NetWorkBuffer::PACKET_TYPE parseBinary2(TC_NetWorkBuffer &in,
                                                    vector<char> &out) {
    return in.parseBufferOf2(out, iMinLength, iMaxLength);
  }

  /**
   * 解析二进制包, 4字节长度(字节序)+包体(iMinLength<包长<iMaxLength,
   * 否则返回PACKET_ERR) Parse binary package, 4 byte length (byte order) +
   * package (iMinLength<package length<iMaxLength, otherwise return PACKET_ERR)
   * 注意: out只返回包体, 不包括头部的4个字节的长度
   * Note: out only returns the package, not including the length of 4 bytes of
   * the head
   * @param in
   * @param out
   * @return
  ...
  static TC_NetWorkBuffer::PACKET_TYPE parseBinary4(TC_NetWorkBuffer &in,
                                                    vector<char> &out) {
   ...
   * @return
   */
  static TC_NetWorkBuffer::PACKET_TYPE parseHttp(TC_NetWorkBuffer &in,
                                                 vector<char> &out);
   ...
   * @return
   */
  static TC_NetWorkBuffer::PACKET_TYPE parseEcho(TC_NetWorkBuffer &in,
                                                 vector<char> &out);
   ...
   * @return
   */
  static TC_NetWorkBuffer::PACKET_TYPE parseJson(TC_NetWorkBuffer &in,
                                                 vector<char> &out);
 ...
 protected:
  ...
  template <typename T>
  TC_NetWorkBuffer::PACKET_TYPE parseBuffer(vector<char> &buffer, T minLength,
                                            T maxLength) {
