...
class TC_GrpcServer : public TC_Http2Server {
 public:
  ...
  int encodeResponse(const shared_ptr<Http2Context> &context,
                     std::string gStatus, vector<char> &out);
   ...
   * @return
   */
  void packGrpcResponse(shared_ptr<TC_GrpcServer::Http2Context> &context,
                        const int status, const string &body);
  ...
  static TC_NetWorkBuffer::PACKET_TYPE parseGrpc(TC_NetWorkBuffer &in,
                                                 vector<char> &out);
...
class TC_GrpcClient : public TC_Http2Client {
 public:
   ...
   * @param in
   */
  TC_NetWorkBuffer::PACKET_TYPE parseResponse(
      TC_NetWorkBuffer &in, pair<int, shared_ptr<TC_HttpResponse>> &out);

  //    int submit(const string &method, const string &path, const map<string,
  //    string> &header, const vector<char> &buff);
