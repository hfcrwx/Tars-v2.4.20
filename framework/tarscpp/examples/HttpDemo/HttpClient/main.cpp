...
// static string httpObj = "TestApp.HttpServer.httpObj@tcp -h 127.0.0.1 -p
// 8081:tcp -h 127.0.0.1 -p 8082:tcp -h 127.0.0.1 -p 8083";
static string httpObj = "TestApp.HttpServer.httpObj@tcp -h 127.0.0.1 -p 8081";
// static string httpObj = "TestApp.HttpServer.httpObj@tcp -h 134.175.105.92 -p
// 8081";
...
void httpCall(int excut_num) {
  ...
  for (int i = 0; i < excut_num; i++) {
    ...
    if (iRet != 0) {
      cout << "pthread id: " << TC_Thread::CURRENT_THREADID()
           << ", iRet:" << iRet << endl;
    }

    ++callback_count;
  }
  cout << "httpCall, succ:" << param.count << "/" << excut_num << ", "
       << TC_TimeProvider::getInstance()->getNowMs() - _iTime << "(ms)" << endl;
}
...
void asyncRpc(int c) {
  ...
  int64_t cost = TC_Common::now2us() - t;
  cout << "asyncRpc2 send:" << cost << "us, avg:" << 1. * cost / c << "us"
       << endl;
}
...
void asyncRpcCallback(int c) {
  ...
  param.servantPrx->tars_set_custom_callback(
      [](ReqMessagePtr msg) { msg->callback->dispatch(msg); });
...
int main(int argc, char* argv[]) {
  try {
    if (argc < 4) {
      cout << "Usage:" << argv[0]
           << "--count=1000 --call=[base|sync|async|callback] --thread=1"
           << endl;
