...
static string sObjName =
    "TestApp.CustomServer.CustomServantObj@tcp -h 127.0.0.1 -t 60000 -p 9400";
...
// The response packet decoding function decodes the data received from the
// server according to the specific format and resolves it to theResponsePacket
static TC_NetWorkBuffer::PACKET_TYPE customResponse(TC_NetWorkBuffer& in,
                                                    ResponsePacket& rsp) {
...
static vector<char> customRequest(RequestPacket& request, Transceiver*) {
  ...
  memcpy(buffer.data() + sizeof(unsigned int), netrequestIdptr,
         sizeof(unsigned int));
  memcpy(buffer.data() + sizeof(unsigned int) * 2, request.sBuffer.data(),
 ...
 public:
  virtual int onDispatch(ReqMessagePtr msg) {
    if (msg->response->iRet != tars::TARSSERVERSUCCESS) {
      ...
      //			cout << "succ:" <<
      //string(msg->response->sBuffer.data(), msg->response->sBuffer.size()) <<
      //endl;
    }
...
void syncCall(int c) {
  ...
  for (int i = 0; i < c; ++i) {
    ...
    try {
      ResponsePacket rsp;
      param.servantPrx->rpc_call(param.servantPrx->tars_gen_requestid(),
                                 "doCustomFunc", buffer.c_str(),
                                 buffer.length(), rsp);
    } catch (exception& e) {
  ...
  cout << "syncCall total:" << cost << "us, avg:" << 1. * cost / c << "us"
       << endl;
}
...
void asyncCall(int c) {
  ...
  for (int i = 0; i < c; ++i) {
    try {
      ...
      param.servantPrx->rpc_call_async(param.servantPrx->tars_gen_requestid(),
                                       "doCustomFunc", buffer.c_str(),
                                       buffer.length(), cb);
    } catch (exception& e) {
      cout << "exception:" << e.what() << endl;
  ...
  int64_t cost = TC_Common::now2us() - t;
  cout << "asyncCall send:" << cost << "us, avg:" << 1. * cost / c << "us"
       << endl;
}
...
int main(int argc, char* argv[]) {
  try {
    if (argc < 6) {
      cout << "Usage:" << argv[0]
           << "--count=1000 --call=[sync|async] --thread=1 --buffersize=1000 "
              "--netthread=1"
           << endl;
    ...
    std::thread print([&] {
      while (callback_count != param.count * param.thread) {
        cout << "Custom:" << param.call
             << ": ----------finish count:" << callback_count << endl;
    ...
    cout << "(pid:" << std::this_thread::get_id() << ")"
         << "(count:" << param.count << ")"
         << "(use ms:" << (TC_Common::now2us() - start) / 1000 << ")" << endl;
