...
static void printResult(int iRequestId, const string &sResponseStr) {
  cout << "request id: " << iRequestId << ", response str: " << sResponseStr
       << endl;
}
...
RecvThread::RecvThread(int second) : _second(second), _bTerminate(false) {
  string sObjName =
      "TestApp.PushServer.TestPushServantObj@tcp -h 127.0.0.1 -t 60000 -p 9300";
  ...
  while (!_bTerminate) {
    {
      try {
        ...
        _prx->rpc_call_async(_prx->tars_gen_requestid(), "printResult",
                             buf.c_str(), buf.length(), cb);
