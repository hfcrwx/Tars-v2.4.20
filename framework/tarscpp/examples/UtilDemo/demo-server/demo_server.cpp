...
class HttpHandle : public TC_EpollServer::Handle {
 public:
  ...
  virtual void initialize() {
    g_logger.debug() << "HttpHandle::initialize: " << std::this_thread::get_id()
                     << endl;
  ...
  virtual void handle(const shared_ptr<TC_EpollServer::RecvContext> &data) {
    try {
      g_logger.debug() << "HttpHandle::handle : " << data->ip() << ":"
                       << data->port() << endl;
  ...
  virtual void handleClose(
    ...
    try {
      g_logger.debug() << "HttpHandle::handleClose : " << data->ip() << ":"
                       << data->port() << endl;

      string current = TC_Common::tostr(data->uid()) + "_" + data->ip() + "_" +
                       TC_Common::tostr(data->port());
...
TC_NetWorkBuffer::PACKET_TYPE parseEcho(TC_NetWorkBuffer &in,
                                        vector<char> &out) {
  TC_EpollServer::Connection *c =
...
class SocketHandle : public TC_EpollServer::Handle {
 public:
   ...
   */
  virtual void initialize() {
    g_logger.debug() << "SocketHandle::initialize: "
  ...
  virtual void handle(const shared_ptr<TC_EpollServer::RecvContext> &data) {
    try {
      cout << "SocketHandle::handle : " << data->ip() << ":" << data->port()
           << endl;
  ...
  virtual void handleClose(
      const shared_ptr<TC_EpollServer::RecvContext> &data) {
    try {
      g_logger.debug() << "SocketHandle::handleClose : " << data->ip() << ":"
                       << data->port();
    } catch (exception &ex) {
      g_logger.error() << "SocketHandle::handle ex:" << ex.what() << endl;
      close(data);
...
class MyServer {
 public:
  MyServer() { _epollServer = new TC_EpollServer(1); };
  ...
  void bindHttp(const std::string &str) {
    TC_EpollServer::BindAdapterPtr lsPtr =
        new TC_EpollServer::BindAdapter(_epollServer);
  ...
  void waitForShutdown() { _epollServer->waitForShutdown(); }
