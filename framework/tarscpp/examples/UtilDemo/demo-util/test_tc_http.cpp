...
TEST_F(HttpTest, testEncodeString)  //此时使用的是TEST_F宏
{
  ...
  cout << "testEncodeString::cost: " << TC_Common::now2us() - t << "us, "
       << 1. * (TC_Common::now2us() - t) / count << "us" << endl;
}
...
TEST_F(HttpTest, testEncodeVector)  //此时使用的是TEST_F宏
{
  ...
  cout << "testEncodeVector::cost: " << TC_Common::now2us() - t << "us, "
       << 1. * (TC_Common::now2us() - t) / count << "us" << endl;
}
...
TEST_F(HttpTest, testEncodeBuffString)  //此时使用的是TEST_F宏
{
  ...
  cout << "testEncodeBuffString::cost: " << TC_Common::now2us() - t << "us, "
       << 1. * (TC_Common::now2us() - t) / count << "us" << endl;
}
...
TEST_F(HttpTest, testDecodeString)  //此时使用的是TEST_F宏
{
  ...
  cout << "testDecodeString::cost: " << TC_Common::now2us() - t << "us, "
       << 1. * (TC_Common::now2us() - t) / count << "us" << endl;
}
...
TEST_F(HttpTest, testDecodeBuffString)  //此时使用的是TEST_F宏
{
  ...
  cout << "testDecodeBuffString::cost: " << TC_Common::now2us() - t << "us, "
       << 1. * (TC_Common::now2us() - t) / count << "us" << endl;
}
...
TEST_F(HttpTest, testCheckRequestString)  //此时使用的是TEST_F宏
{
  ...
  cout << "testCheckRequestString::cost: " << TC_Common::now2us() - t << "us, "
       << 1. * (TC_Common::now2us() - t) / count << "us" << endl;
}
...
TEST_F(HttpTest, testCheckRequestBuff)  //此时使用的是TEST_F宏
{
  ...
  cout << "testCheckRequestBuff::cost: " << TC_Common::now2us() - t << "us, "
       << 1. * (TC_Common::now2us() - t) / count << "us" << endl;
}
...
TEST_F(HttpTest, testWeb)  //此时使用的是TEST_F宏
{
  ...
  TC_HttpRequest stHttpReq;
  //	stHttpReq.setCacheControl("no-cache");
  //	stHttpReq.setUserAgent("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_0)
...
void testCookie(const string &sRspURL, const string &sReqURL,
                const vector<string> &vsCookie) {
  cout << sRspURL << "=>" << sReqURL << "-----------------------------------"
       << endl;
  ...
  cout << "All Cookie:" << sRspURL << "-----------------------------------"
       << endl;

  list<TC_HttpCookie::Cookie>::iterator it = vCookie.begin();

  while (it != vCookie.end()) {
    cout << TC_Common::tostr(it->_data.begin(), it->_data.end(), "; ") << ", "
         << it->_expires << ", " << it->_path << endl;
...
TEST_F(HttpTest, testCookie)  //此时使用的是TEST_F宏
{
  ...
  s += "Set-Cookie: n1=1; a=1; c=d; Path=/; Domain=qq.com; Expires=" + gmt +
       "\r\n";
  s += "Set-Cookie: n2=2; a=0; c=d; Path=/abc/def; Domain=.qq.com; Expires=" +
       gmt + "\r\n";
  s += "Set-Cookie: n3=3; a=5; c=d; Path=/abc/def/aaa; Domain=.qq.com; "
       "Expires=" +
       gmt + "\r\n";
  ...
  s += "Set-Cookie: n5=5; a=2; c=d; Path=/; Domain=.qq.com; Expires=" + gmt +
       "\r\n";
  s +=
      "Set-Cookie: n6=6; c=3; Path=/; Domain=y.qq.com; Expires=" + gmt + "\r\n";
  ...
  s += "Set-Cookie: n9=7; c=4; Path=/; Domain=.x.y.qq.com; Expires=" + gmt +
       "\r\n";
  s +=
      "Set-Cookie: n10=7; c=4; Path=/; Domain=qqq.com; Expires=" + gmt + "\r\n";
  ...
  testCookie("http://www.qq.com/abc/def/aaa", "http://www.qq.com/abc/def/aaa",
             vsCookie);
