...
vector<tars::TokenResponse> AuthImp::getTokens(
    const tars::TokenRequest& request, tars::TarsCurrentPtr current) {
  vector<tars::TokenResponse> rsp;
  for (vector<string>::const_iterator it(request.vObjName.begin());
       it != request.vObjName.end(); ++it) {
    string sSql;
    sSql += "select application, server, token from " + tableName +
            " where objname = '";
    sSql += *it;
    sSql += "';";
...
#endif
tars::ApplyTokenResponse AuthImp::applyToken(
    const tars::ApplyTokenRequest& request, tars::TarsCurrentPtr current) {
  // gen secret key;
  char token[32] = "";
  ...
  sql << "insert into " + tableName +
             " (application, server, objname, token) values('";
  sql << request.sKey.sApplication << "','" << request.sKey.sServer << "','"
      << request.sKey.sObjName << "','";
  sql << sToken << "');";
...
int AuthImp::deleteToken(const tars::DeleteTokenRequest& request,
  ...
  sql << request.sKey.sApplication << "' and server='" << request.sKey.sServer
      << "' and objname='" << request.sKey.sObjName << "';";
