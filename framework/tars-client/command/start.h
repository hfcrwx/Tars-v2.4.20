...
void start(TarsClient* tarsClient, TC_Option& option) {
  ...
  AdminRegPrx adminPrx =
      tarsClient->getCommunicator()->stringToProxy<AdminRegPrx>(
          "tars.tarsAdminRegistry.AdminRegObj");
  ...
  try {
    ...
    // ServantPrx prx =
    // tarsClient->getCommunicator()->stringToProxy<ServantPrx>(obj);
    // prx->tars_ping();
