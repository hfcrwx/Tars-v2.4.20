  ...
  while (1) {
    ...
    if (c == '\\') {
      ...
      if (c == '\\' || c == '\"' || c == '/')
        ...
        //                char s[64];
        //                snprintf(s, sizeof(s), "get string
        //                error1(\\u)[pos:%u], code:%x",
        ...
        //                    snprintf(s, sizeof(s), "get string
        //                    error1(\\u)[pos:%u], code:%x",
        //                    (uint32_t)reader.getCur(), iCode); throw
        //                    TC_Json_Exception(s);
        //                }
        //                pChar+=4;
...
uint32_t TC_Json::getHex(BufferJsonReader& reader) {
  ...
  for (int iLoop = 0; iLoop < 4; iLoop++) {
    ...
    if (c >= 'a' && c <= 'f')
      ...
      char s[64];
      snprintf(s, sizeof(s), "get string error3(\\u)[pos:%u]",
               (uint32_t)reader.getCur());
      throw TC_Json_Exception(s);
    }
  }
...
void TC_Json::writeObj(const JsonValueObjPtr& p, string& ostr) {
  ostr += "{ ";
  unordered_map<string, JsonValuePtr>::const_iterator it(p->value.begin()),
      it_end(p->value.end());
  while (it != it_end) {
...
void TC_Json::writeArray(const JsonValueArrayPtr& p, string& ostr) {
  ostr += "[ ";
  vector<JsonValuePtr>::const_iterator it(p->value.begin()),
      it_end(p->value.end());
  while (it != it_end) {
    writeValue(*it, ostr);
    if (++it != it_end) {
...
void TC_JsonWriteOstream::writeString(const JsonValueStringPtr& p,
                                      ostream& sReturn) {
...
void TC_JsonWriteOstream::writeArray(const JsonValueArrayPtr& p,
                                     ostream& ostr) {
  ostr << "[ ";
  vector<JsonValuePtr>::const_iterator it(p->value.begin()),
      it_end(p->value.end());
...
void TC_JsonWriteOstream::writeBoolean(const JsonValueBooleanPtr& p,
                                       ostream& ostr) {
  if (p->value)
    ostr << "true";
  else
