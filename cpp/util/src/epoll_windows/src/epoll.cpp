...
class epoll_port_data_t {
 ...
 public:
  ...
  int epoll_close();
  int epoll_wait(OVERLAPPED_ENTRY *entries, ULONG count,
...
epoll_sock_data_t::epoll_sock_data_t(SOCKET sock, SOCKET base_sock,
                                     SOCKET peer_sock) {
  _sock = sock;
...
int epoll_sock_data_t::submit() {
  DWORD afd_events = AFD_POLL_ABORT | AFD_POLL_DISCONNECT |
...
SOCKET epoll_port_data_t::get_peer_socket(WSAPROTOCOL_INFOW *protocol_info) {
  ...
  for (int i = 0; i < ARRAY_COUNT(AFD_PROVIDER_IDS); i++) {
    if (memcmp((void *)&protocol_info->ProviderId, (void *)&AFD_PROVIDER_IDS[i],
...
SOCKET epoll_port_data_t::create_peer_socket(WSAPROTOCOL_INFOW *protocol_info) {
  SOCKET sock = WSASocketW(protocol_info->iAddressFamily,
...
int epoll_port_data_t::epoll_add(SOCKET sock, struct epoll_event *ev) {
  ...
  WSAIoctl(sock, SIO_BASE_HANDLE, NULL, 0, &base_sock, sizeof base_sock, &bytes,
           NULL, NULL);

  /* Obtain protocol information about the socket. */
  int len = sizeof protocol_info;
  if (getsockopt(base_sock, SOL_SOCKET, SO_PROTOCOL_INFOW,
...
int epoll_port_data_t::epoll_close() {
  ...
  while (!_sock_data_tree.empty()) {
    ...
    DWORD result = GetQueuedCompletionStatusEx(
        _iocp, entries, ARRAY_COUNT(entries), &count, 0, FALSE);
    if (!result) {
    ...
    for (ULONG i = 0; i < count; i++) {
      epoll_op_t *op =
          CONTAINING_RECORD(entries[i].lpOverlapped, epoll_op_t, _overlapped);
...
int epoll_port_data_t::epoll_wait(OVERLAPPED_ENTRY *entries, ULONG count,
                                  struct epoll_event *events) {
  std::lock_guard<std::mutex> lck(_mutex);
  ...
  for (ULONG i = 0; i < count; i++) {
    ...
    // cout << "wait ok, handles:" << op->_poll_info.NumberOfHandles << ",
    // events:" << afd_events << endl;
...
int epoll_wait(epoll_t port_handle, struct epoll_event *events, int maxevents,
  ...
  DWORD result = GetQueuedCompletionStatusEx(
      port_data->getHandle(), entries, max_entries, &count, gqcs_timeout, TRUE);
