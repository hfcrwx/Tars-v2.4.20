...
void TC_NetWorkBuffer::addBuffer(
    const shared_ptr<TC_NetWorkBuffer::Buffer> &buff) {
...
TC_NetWorkBuffer::buffer_iterator TC_NetWorkBuffer::find(const char *str,
                                                         size_t length) {
...
bool TC_NetWorkBuffer::empty() const { return _length == 0; }

size_t TC_NetWorkBuffer::getBufferLength() const { return _length; }
...
uint8_t TC_NetWorkBuffer::getValueOf1() const { return getValue<uint8_t>(); }

uint16_t TC_NetWorkBuffer::getValueOf2() const { return getValue<uint16_t>(); }

uint32_t TC_NetWorkBuffer::getValueOf4() const { return getValue<uint32_t>(); }

TC_NetWorkBuffer::PACKET_TYPE TC_NetWorkBuffer::parseBufferOf1(
    vector<char> &buffer, uint8_t minLength, uint8_t maxLength) {
  return parseBuffer<uint8_t>(buffer, minLength, maxLength);
}

TC_NetWorkBuffer::PACKET_TYPE TC_NetWorkBuffer::parseBufferOf2(
    vector<char> &buffer, uint16_t minLength, uint16_t maxLength) {
  return parseBuffer<uint16_t>(buffer, minLength, maxLength);
}

TC_NetWorkBuffer::PACKET_TYPE TC_NetWorkBuffer::parseBufferOf4(
    vector<char> &buffer, uint32_t minLength, uint32_t maxLength) {
...
TC_NetWorkBuffer::PACKET_TYPE TC_NetWorkBuffer::parseHttp(TC_NetWorkBuffer &in,
                                                          vector<char> &out) {
...
TC_NetWorkBuffer::PACKET_TYPE TC_NetWorkBuffer::parseEcho(TC_NetWorkBuffer &in,
                                                          vector<char> &out) {
  try {
    out = in.getBuffers();
    in.clearBuffers();
...
TC_NetWorkBuffer::PACKET_TYPE TC_NetWorkBuffer::parseJson(TC_NetWorkBuffer &in,
                                                          vector<char> &out) {
  ...
  return TC_NetWorkBuffer::
      PACKET_ERR;  //返回-1表示收到包协议错误，框架会自动关闭当前连接
}
