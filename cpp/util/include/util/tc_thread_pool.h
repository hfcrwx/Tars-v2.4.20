 ...
 *     cout << i << endl;
 * }
 * tpool.start(testFunction, 5);
 * //start的第一函数是std::bind返回的函数(std::function), 后面跟参数
 * //将任务丢到线程池中
 ...
 * tpool.stop();
 * 注意:
 * TC_ThreadPool::exec执行任务返回的是个future, 因此可以通过future异步获取结果,
...
struct TC_ThreadPool_Exception : public TC_Exception {
  ...
  TC_ThreadPool_Exception(const string& buffer, bool err)
...
class TC_ThreadPoolHash {
 public:
   ...
   */
  template <class F, class... Args>
  auto exec(const string& hashkey, F&& f, Args&&... args)
      -> std::future<decltype(f(args...))> {
   ...
   */
  template <class F, class... Args>
  auto exec(const string& hashkey, int64_t timeoutMs, F&& f, Args&&... args)
      -> std::future<decltype(f(args...))> {
    ...
    if (thread) {
      ...
      throw TC_ThreadPool_Exception(
          "[TC_ThreadPoolHash::start] no worker thread!");
    }
  }
