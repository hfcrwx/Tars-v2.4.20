...
class TC_Timer {
 protected:
  struct Func {
    Func(uint64_t fireMillseconds, uint32_t uniqueId)
        : _fireMillseconds(fireMillseconds), _uniqueId(uniqueId) {}
    ~Func() {}
 ...
 public:
  ...
  int64_t postDelayed(int64_t delayMillseconds, F &&f, Args &&... args) {
    uint64_t fireMillseconds =
        TC_TimeProvider::getInstance()->getNowMs() + delayMillseconds;

    return post(create(fireMillseconds, 0, "", f, args...));
  }

  /**
   * @brief 重复repeatTime(毫秒)执行一次, 注意repeat是上一次执行完,
   * 才会重新触发下一次事件的计时
   ...
   */
  template <class F, class... Args>
  int64_t postRepeated(int64_t repeatTime, bool execNow, F &&f,
                       Args &&... args) {
    uint64_t fireMillseconds;
  ...
  /**
   * @brief
   * @param cronexpr, crontab 语法  <seconds> <minutes> <hours> <days of month>
   * <months> <days of week> ，具体例子参考 tc_cron.h
   * @return 返回事件Id
   */
  // CRON	Description
  // * * * * * *	Every second
 ...
 protected:
  template <class F, class... Args>
  shared_ptr<Func> create(int64_t fireMillseconds, int64_t repeatTime,
                          const string &cronexpr, F &&f, Args &&... args) {
    //定义返回值类型
    ...
    shared_ptr<Func> fPtr =
        std::make_shared<Func>(fireMillseconds, genUniqueId());
      ...
      fPtr->_func = [task, this, wPtr, repeatTime]() {
        ...
        if (p) {
          if (this->exist(p->_uniqueId, true)) {
            if (p->_cron.isset) {
              p->_fireMillseconds =
                  TC_Cron::nextcron(p->_cron, p->_fireMillseconds / 1000) *
                  1000;
              this->post(p);
            } else if (repeatTime > 0) {
              p->_fireMillseconds =
                  TC_TimeProvider::getInstance()->getNowMs() + repeatTime;
              this->post(p);
            }
