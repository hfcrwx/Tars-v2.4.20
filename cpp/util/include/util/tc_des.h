 ...
 * @brief Use des/3des to encrypt and decrypt the source code, independent of
 * any library.
 *
 ...
 * For des encryption, use 8-bit key. If it is less than 8 bits, right complete
 * 0x00. If it is more than 8 bits, only the left 8 bits are valid.
 *
 * 加密内容8位补齐,补齐方式为:少1位补一个0x01,少2位补两个0x02,...
 * The encrypted content follow 8-bit completion, the completion method is: less
 * 1 bit to complete one 0x01, less 2 bits to complete two 0x02,...
 *
 * 本身已8位对齐的,后面补八个0x08.
 ...
 * For 24-bit key, if it is less than 24 bits, right complete 0x00. If it is
 * more than 24 bits, only the left 24 bits are valid.
 *
 * 加密内容8位补齐，补齐方式为：少1位补一个0x01,少2位补两个0x02,...
 * The encrypted content follow 8-bit completion, the completion method is: less
 * 1 bit to complete one 0x01, less 2 bits to complete two 0x02,...
...
class TC_Des {
 public:
   ...
   * @return        string, the decrypted content, if decryption failed, return
   * null.
   */
   ...
   * @return         string, the dcrypted content, if decryption failed, return
   * null.
   */
 ...
 protected:
   ...
   * @param mode  mode : 0(encryption), 1(decryption)
   */
  static void des3key(const char *key, short mode, uint32_t *KnL, uint32_t *KnR,
                      uint32_t *Kn3);
   ...
   * @param to    encrypt and decrypt the only 8 bits.
   */
  static void des3(const char *from, char *into, uint32_t *KnL, uint32_t *KnR,
                   uint32_t *Kn3);
