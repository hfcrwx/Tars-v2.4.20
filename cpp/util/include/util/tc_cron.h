...
struct TC_CronException : public TC_Exception {
 public:
  ...
  //	TC_CronException(const string& buffer, bool err) : TC_Exception(buffer,
  //err) {};
...
class TC_Cron {
 ...
 public:
   ...
   * @return  cron object
   */
  // 字段分别为  <seconds> <minutes> <hours> <days of month> <months> <days of
  // week> The fields are: <seconds> <minutes> <hours> <days of month> <months>
  // <days of week> 通配符以及含义 Wildcards and their meanings: *	all
  // values	selects all values within a field -	range	specify ranges
  // ,	comma	specify additional values
  // /	slash	speficy increments
  // DAYS = { "SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT" };
  // MONTHS = { "NIL", "JAN", "FEB", "MAR", "APR", "MAY", "JUN", "JUL", "AUG",
  // "SEP", "OCT", "NOV", "DEC" }; 各类例子 Examples: CRON	Description
  // * * * * * *	Every second
  // */5 * * * * *	Every 5 seconds
  // 0 */5 */2 * * *	Every 5 minutes, every 2 hours
  // 0 */2 */2 * */2 */2	Every 2 minutes, every 2 hours, every 2 days of the
  // week, every 2 months 0 15 10 * * * *	10:15 AM every day 0 0/5 14 * *
  // *	Every 5 minutes starting at 2 PM and ending at 2:55 PM, every day 0
  // 10,44 14 * 3 WED	2:10 PM and at 2:44 PM every Wednesday of March 0 15 10
  // * * MON-FRI	10:15 AM every Monday, Tuesday, Wednesday, Thursday and
  // Friday 0 0 12 1/5 * *	12 PM every 5 days every month, starting on the
  // first day of the month 0 11 11 11 11 *	Every November 11th at 11:11 AM
   ...
   * @return If INVALID_TIME is returned, it is the wrong time. Otherwise the
 ...
 protected:
   ...
   */
  static std::string replaceOrdinals(
      std::string text, const std::vector<std::string>& replacement);

  /**
   * @brief 获取时间范围，用于计算通配符
   * @brief Gets the time range for calculating wildcards.
   */
  static std::pair<cron_int, cron_int> makeRange(std::string field,
                                                 cron_int minval,
                                                 cron_int maxval);
   ...
   */
  template <size_t N>
  static void setCronField(std::string value, std::bitset<N>& target,
                           cron_int minval, cron_int maxval);
   ...
   */
  template <size_t N>
  static size_t findNext(const std::bitset<N>& target, std::tm& tmt,
                         unsigned int minimum, unsigned int maximum,
                         unsigned int value, CronField field,
                         CronField next_field,
                         const std::bitset<7>& marked_fields);
  static bool findNext(const TC_Cron& cron, std::tm& tmt, size_t dot);
  static size_t findNextDay(std::tm& tmt, std::bitset<31> const& days_of_month,
                            size_t day_of_month,
   ...
   */
  template <size_t N>
  static size_t nextSetbit(const std::bitset<N>& target, size_t minimum,
...
template <size_t N>
void TC_Cron::setCronField(std::string value, std::bitset<N>& target,
                           cron_int minval, cron_int maxval) {
  if (value.length() > 0 && value[value.length() - 1] == ',')
    throw TC_CronException(
        "[TC_Cron::setCronField] value cannot end with comma");
  ...
  for (auto const& field : fields) {
    if (!contains(field, '/')) {
      ...
      if (parts.size() != 2)
        throw TC_CronException(
            "[TC_Cron::setCronField] incrementer must have two fields");
      ...
      if (delta <= 0)
        throw TC_CronException(
            "[TC_Cron::setCronField] incrementer must be a positive value");
...
template <size_t N>
size_t TC_Cron::nextSetbit(const std::bitset<N>& target, size_t minimum,
                           size_t maximum, size_t offset) {
...
template <size_t N>
size_t TC_Cron::findNext(const std::bitset<N>& target, std::tm& tmt,
                         unsigned int minimum, unsigned int maximum,
                         unsigned int value, CronField field,
                         CronField next_field,
                         const std::bitset<7>& marked_fields) {
