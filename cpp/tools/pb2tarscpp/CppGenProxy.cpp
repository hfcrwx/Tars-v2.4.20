...
static std::string GenSyncCall(
  ...
  out += ToCppNamespace(method->output_type()->full_name()) + " " +
         ...
         "& req, const std::map<std::string, std::string>& context = "
         "TARS_CONTEXT(), " +
  ...
  out +=
      "shared_ptr<tars::ResponsePacket> rep = tars_invoke(tars::TARSNORMAL, "
      "\"" +
      method->name() + "\", _vc, context, _mStatus);" + LineFeed(indent);
  ...
  out += ToCppNamespace(method->output_type()->full_name()) + " _ret;" +
         LineFeed(indent);
  out += "_ret.ParseFromArray(rep->sBuffer.data(), rep->sBuffer.size());" +
         LineFeed(indent) + "return _ret;";
  out += LineFeed(--indent) + "}";
  out += LineFeed(indent);
  out += LineFeed(indent);
...
static std::string GenAsyncCall(
    const ::google::protobuf::MethodDescriptor* method, const std::string& name,
    const std::string& pkg, int indent) {
  std::string out;
  out.reserve(8 * 1024);

  out += "void async_" + method->name() + "(" + name +
         "PrxCallbackPtr callback, const " +
         ToCppNamespace(method->input_type()->full_name()) +
         "& req, const std::map<std::string, std::string>& context = "
         "TARS_CONTEXT())" +
         LineFeed(indent);
  ...
  out += "std::string _os;" + LineFeed(indent) +
         ...
         "std::vector<char> _vc(_os.begin(), _os.end());" + LineFeed(indent);
  out += "std::map<std::string, std::string> _mStatus;" + LineFeed(indent);
  out += "tars_invoke_async(tars::TARSNORMAL, \"" + method->name() +
         "\", _vc, context, _mStatus, callback);";
  out += LineFeed(--indent) + "}";
  out += LineFeed(indent);
...
std::string GenPrx(const ::google::protobuf::ServiceDescriptor* desc,
  ...
  out += "public:";
  out += LineFeed(++indent);
  out += "typedef std::map<std::string, std::string> TARS_CONTEXT;" +
         LineFeed(indent);
