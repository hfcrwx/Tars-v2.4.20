...
static std::string GenMethods(
    const ::google::protobuf::MethodDescriptor* method, const std::string& pkg,
    int indent) {
  std::string out;
  out.reserve(8 * 1024);

  out += "virtual " + ToCppNamespace(method->output_type()->full_name()) + " " +
         method->name() + "(const " +
         ToCppNamespace(method->input_type()->full_name()) +
         "& , tars::TarsCurrentPtr current) = 0;" + LineFeed(indent);
  out += "static void async_response_" + method->name() +
         "(tars::TarsCurrentPtr current, const " +
         ToCppNamespace(method->output_type()->full_name()) + "&_ret)" +
         LineFeed(indent);
  out += "{" + LineFeed(++indent);
...
static std::string GenDispatchCase(
    const ::google::protobuf::MethodDescriptor* method, const std::string& pkg,
    int indent) {
  std::string out;
  out.reserve(8 * 1024);
  ...
  out += ToCppNamespace(method->input_type()->full_name()) + " req;" +
         LineFeed(indent);
  out +=
      "req.ParseFromArray(&_current->getRequestBuffer()[0], "
      "_current->getRequestBuffer().size());" +
      LineFeed(indent);
  out += LineFeed(indent);

  out += ToCppNamespace(method->output_type()->full_name()) +
         " _ret = " + method->name() + "(req, _current);" + LineFeed(indent);
...
std::string GenServant(const ::google::protobuf::ServiceDescriptor* desc,
                       int indent) {
  std::string out;
  out.reserve(8 * 1024);
  ...
  out += LineFeed(indent);
  out +=
      "int onDispatch(tars::TarsCurrentPtr _current, std::vector<char>& "
      "_sResponseBuffer)";
  out += LineFeed(indent);
  out += "{";
  out += LineFeed(++indent);
